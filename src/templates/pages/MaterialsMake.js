...
class MaterialsMake extends Component {
	...
	async onSubmit() {
		...
		const response = await fetch(HOST + 'api/materials/make/', {
			method: 'POST',
			credentials: 'include',
		...
		if (response.ok) {
			...
			if (responseData.success){
				this.setState({
					"redirectPath": `/person/materials/detail/?id=${responseData.body.id}`,
				});
			} else {
		...
		return <>
			<Container>
				...
				<div className="column-flex" >
					...
					<Input handle={this.onChangeInput} name="name_lesson" label="Название"/>
					<textarea onChange={this.onChangeInput} name="description_lesson" placeholder="Описание..." />
					<input
						type="file" 
